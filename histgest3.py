...
global for_cnt_ref, for_cnt_bgr_ref, count
count = 0
lim = 1
...
while cap.isOpened():
    ...
    if count > 10:
        #print(np.amax(np.abs(for_contour[w//8:(w*7)//8, h//2:] - for_cnt_ref[w//8:(w*7)//8, h//2:])))
        for_contour[w//8:(w*7)//8, h//2:] = np.where(np.abs(for_contour[w//8:(w*7)//8, h//2:] - for_cnt_ref[w//8:(w*7)//8, h//2:]) < lim, 0, for_contour[w//8:(w*7)//8, h//2:]) 
        # for_contour[w//8:(w*7)//8, h//2:]=cv2.filter2D(for_contour[w//8:(w*7)//8, h//2:], -1, kernel)
        # for_contour[w//8:(w*7)//8, h//2:] = cv2.dilate(for_contour[w//8:(w*7)//8, h//2:], kernel_, iterations=1)
        # for_contour[w//8:(w*7)//8, h//2:] = cv2.morphologyEx(for_contour[w//8:(w*7)//8, h//2:], cv2.MORPH_CLOSE, kernel_, iterations=1)
    for_contour_bgr[:, :(h*9)//16, :] = 0
    for_contour_bgr[:w//8, h//2:, :] = 0
    for_contour_bgr[(w*7)//8 :, h//2:, :] = 0
    if count > 10:
        for_contour_bgr[w//8:(w*7)//8, h//2:,:] = np.where(np.abs(for_contour_bgr[w//8:(w*7)//8, h//2:, :] - for_cnt_bgr_ref[w//8:(w*7)//8, h//2:, :]) < lim, 0, for_contour_bgr[w//8:(w*7)//8, h//2:, :])
    ...
    if count == 10:
        for_cnt_ref = for_contour
        for_cnt_bgr_ref = for_contour_bgr
    elif count > 10:
        count = 20
        cv2.imshow('for_contour_bgr', for_contour_bgr)
        cv2.imshow("for_contour", for_contour)    
        media_player.play()
        prev = direction
